...
func NewRoleHierarchyService(repo Repository, cache HierarchyCache, logger logging.Logger) *RoleHierarchyService {
...
func (rhs *RoleHierarchyService) GetRoleAncestors(ctx context.Context, roleID xid.ID, orgID *xid.ID) ([]*Role, error) {
	hierarchy, err := rhs.BuildHierarchy(ctx, orgID)
	if err != nil {
		return nil, err
	}
...
func (rhs *RoleHierarchyService) GetRoleDescendants(ctx context.Context, roleID xid.ID, orgID *xid.ID) ([]*Role, error) {
	hierarchy, err := rhs.BuildHierarchy(ctx, orgID)
	if err != nil {
		return nil, err
	}
...
func (rhs *RoleHierarchyService) getRolesForOrganization(ctx context.Context, orgID *xid.ID) ([]*Role, error) {
	params := ListRolesParams{}
	if orgID != nil {
		params.OrgID.IsSet = true
		params.OrgID.Value = *orgID
	}
	...
	// Convert ent.Role to Role DTO
	roles := make([]*Role, len(result.Data))
...
func (rhs *RoleHierarchyService) buildRelationships(ctx context.Context, hierarchy *RoleHierarchy, roles []*Role) error {
...
func (rhs *RoleHierarchyService) collectDescendants(node *RoleHierarchyNode, descendants *[]*Role) {
	for _, child := range node.Children {

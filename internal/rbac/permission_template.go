...
func NewPermissionTemplateService(repo Repository, discovery *ResourceDiscoveryService, logger logging.Logger) *PermissionTemplateService {
...
func (pts *PermissionTemplateService) CreateTemplate(ctx context.Context, input CreateTemplateInput) (*PermissionTemplate, error) {
	...
	// Store template (you'll need to add this to your schema/repository)
...
func (pts *PermissionTemplateService) ApplyTemplate(ctx context.Context, input ApplyTemplateInput) (*Role, error) {
	// Get template
	...
	role, err := pts.repo.CreateRoleAdvanced(ctx, CreateRoleRequest{
		Name:           roleInput.Name,
	...
	for _, templatePerm := range template.Permissions {
		...
		err = pts.repo.AddPermissionToRole(ctx, role.ID, permission.ID)
		if err != nil {

...
func (s *ssoService) InitiateSSOLogin(ctx context.Context, req model.SSOLoginRequest) (*model.SSOLoginResponse, error) {
	...
	// Delegate to appropriate ssoService based on provider type
	var authURL string
...
func (s *ssoService) TestConnection(ctx context.Context, req model.TestSSOConnectionRequest) (*model.TestSSOConnectionResponse, error) {
	...
	// Delegate to appropriate ssoService
	var testErr error
	switch strings.ToLower(provider.Type) {
	case "saml":
...
// GetSSOStats gets SSO statistics
...
// validateProviderConfig validates SSO provider configuration
func (s *ssoService) validateProviderConfig(providerType string, config model.IdentityProviderConfig) error {
	switch strings.ToLower(providerType) {
	case "saml":
		return s.samlService.ValidateConfig(config)
	case "oidc", "oauth2":
		return s.oidcService.ValidateConfig(config)
	default:
...
func (s *ssoService) findOrCreateUser(ctx context.Context, provider *model.IdentityProvider, userInfo *SSOUserInfo) (*ent.User, bool, error) {
	...
	existingUser, err := s.userRepo.GetByExternalID(ctx, userInfo.ID, provider.Type, user.UserTypeExternal, &provider.OrganizationID)
	if err == nil {
	...
	if provider.AutoProvision {
		existingUser, err := s.userRepo.GetByEmail(ctx, userInfo.Email, user.UserTypeExternal, &provider.OrganizationID)
		if err == nil {
	...
	// Create organization membership if default role is specified
	if provider.DefaultRole != "" {
		// This would typically create a membership with the default role
		// For now, we'll skip this as it requires role resolution
	}
...
func (s *ssoService) convertEntProviderToModel(entProvider *ent.IdentityProvider) *model.IdentityProvider {
	return &model.IdentityProvider{
		...
		Config: model.IdentityProviderConfig{
			ClientID:            entProvider.ClientID,
			ClientSecret:        entProvider.ClientSecret,
			AuthURL:             entProvider.AuthorizationEndpoint,
			TokenURL:            entProvider.TokenEndpoint,
			UserInfoURL:         entProvider.UserinfoEndpoint,
			JWKSUrl:             entProvider.JwksURI,
			Scopes:              nil,
			EntityID:            "",
			SSOUrl:              "",
			SLOUrl:              "",

...
func (s *passwordService) findUserByEmail(ctx context.Context, email string) (*model.User, error) {
	...
	user, err := s.userService.GetUserByEmail(ctx, email, entUser.UserTypeExternal, nil)
	if err == nil && user != nil {
		return user, nil
	}

	// Try internal user
	user, err = s.userService.GetUserByEmail(ctx, email, entUser.UserTypeInternal, nil)
	if err == nil && user != nil {
		return user, nil
	}

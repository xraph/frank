...
func (sc *SystemCommands) systemStats(cmd *cobra.Command, args []string) error {
	...
	internalUsers, _ := db.User.Query().Where(user.UserTypeEQ(user.UserTypeInternal)).Count(sc.base.Ctx)
	externalUsers, _ := db.User.Query().Where(user.UserTypeEQ(user.UserTypeExternal)).Count(sc.base.Ctx)
	...
	platformOrgs, _ := db.Organization.Query().Where(organization.OrgTypeEQ(organization.OrgTypePlatform)).Count(sc.base.Ctx)
	customerOrgs, _ := db.Organization.Query().Where(organization.OrgTypeEQ(organization.OrgTypeCustomer)).Count(sc.base.Ctx)
...
func (sc *SystemCommands) generateAPIKey(cmd *cobra.Command, args []string) error {
	...
	user, err := userService.GetUserByIdentifier(sc.base.Ctx, userEmail, user.UserTypeEndUser)
	if err != nil {

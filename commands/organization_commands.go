...
import (
	...
	organization2 "github.com/juicycleff/frank/internal/services/organization"
	"github.com/rs/xid"
...
func (oc *OrganizationCommands) listOrganizations(cmd *cobra.Command, args []string) error {
	...
	if orgType != "" {
		query = query.Where(organization.OrgTypeEQ(organization.OrgType(orgType)))
	}
...
func (oc *OrganizationCommands) createOrganization(cmd *cobra.Command, args []string) error {
	...
	createReq := &model.CreateOrganizationRequest{
		...
		Slug:       slug,
		Domain:     &domain,
		OrgType:    organization.OrgType(orgType),
		Plan:       plan,

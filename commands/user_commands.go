...
func (uc *UserCommands) listUsers(cmd *cobra.Command, args []string) error {
	...
	if userType != "" {
		query = query.Where(user.UserTypeEQ(user.UserType(userType)))
	}
...
func (uc *UserCommands) createUser(cmd *cobra.Command, args []string) error {
	...
	if isAdmin {
		rbacService := uc.base.Container.RBACService()
		_, err = rbacService.AssignRoleToUser(uc.base.Ctx, rbac.AssignRoleToUserInput{
			UserID: newUser.ID,

...
func VerifyPassword(hashedPassword, password string) error {
	...
	if err != nil {
		...
		return errors.Wrap(err, errors.CodeCryptoError, "failed to verify password")
	}
...
// DeriveKeyWithPBK derives a key from a password using PBKDF2
func DeriveKeyWithPBK(password, salt string, iterations, keyLen int) string {
	if salt == "" {
...
func ParseDerivedKey(derivedKey string) (algorithm string, iterations int, salt string, key string, err error) {
	...
	if err != nil || parsed != 4 {
		return "", 0, "", "", errors.Wrap(err, errors.CodeCryptoError, "failed to parse derived key")
	}
	return
}

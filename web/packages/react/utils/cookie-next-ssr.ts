...
import {deleteCookie, getCookie, getCookies, hasCookie, setCookie} from 'cookies-next/server';

export class NextServerCookieHandler implements CookieHandler {
    constructor(private req: NextRequest, private res: NextResponse) {}

    async setCookie(name: string, value: string, opts: Options = {}): Promise<void> {
    ...
    async getCookies(): Promise<{ [key: string]: string } | Partial<{ [key: string]: string }>> {

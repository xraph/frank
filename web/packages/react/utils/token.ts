...
export const setTokenData = (data: TokenData, cookieHandler?: CookieHandler): void => {
	...
	if (getConfig().tokenStorageType === 'cookie' && typeof document !== 'undefined') {
...
export const isTokenExpired = (cookie?: string, response?: Response): boolean => {
...
export const getSessionTokenFromCookie = (cookieString: string): string | null => {
...
// Get token from session cookie (for server-side use)
export const getRemoteSessionTokenFromCookie = (cookieString: string, cookieName?: string): string | null => {
	if (!cookieString) return null;
...
// Check if token from cookie is valid and not expired
export const isRemoteAuthenticatedFromCookie = (cookieString: string, cookieName?: string): boolean => {

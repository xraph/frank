...
import { useIdentityVerification } from "@/hooks/use-invitation";
...
export const VerificationMethods = withErrorBoundary(
			...
			switch (status) {
				...
				case "verifying":
					return {
						...
						icon: method === "email" ? EnvelopeIcon : DevicePhoneMobileIcon,
					};
				default:
					return {
				...
				<ButtonGroup className="w-full">
					{availableMethods.includes("email") && (
						<Button
							...
							startContent={React.createElement(getMethodStatus("email").icon, {
					...
					{availableMethods.includes("phone") && (
						<Button
							...
							startContent={React.createElement(getMethodStatus("phone").icon, {
...
export const VerificationProgressComponent = withErrorBoundary(
	function VerificationProgressComponent({
		steps,
		currentStep,
		className,
							...
							<div key={step.id} className="flex items-center">
								...
								{!isLast && (
									<div className="ml-4 flex-shrink-0">
										<div
											className={`h-0.5 w-8 ${step.status === "completed" ? "bg-success" : "bg-default-200"}`}
										/>
									</div>
								)}
							</div>
...
export const VerificationSuccess = withErrorBoundary(
...
export const IdentityVerification = withErrorBoundary(
	function IdentityVerification({
		...
		const getVerificationSteps = () => {
			return methods.map((method) => ({
				...
				status: completedMethods.includes(method)
					? ("completed" as const)
					: verification.activeMethod === method
						? ("active" as const)
						: (method === "email"
									? verification.emailStatus
									: verification.phoneStatus) === "error"
							? ("error" as const)
							: ("pending" as const),
			}));
		};
		...
		const renderContent = () => {
			...
			return (
				<div className="space-y-6">
					...
					<div className="space-y-6">
						{/* Email verification */}
						{(selectedMethod === "email" || requireBoth) &&
							methods.includes("email") &&
							!completedMethods.includes("email") && (
								<div
									className={
										requireBoth
											? ""
						...
						{/* Phone verification */}
						{(selectedMethod === "phone" || requireBoth) &&
					...
					{requireBoth &&
						completedMethods.length > 0 &&
						completedMethods.length < methods.length && (
...
export const IdentityVerificationCard = withErrorBoundary(
	function IdentityVerificationCard({
		...
		return (
			<Card
				className={`max-w-lg mx-auto ${props.className || ""}`}
				variant={variant}
				radius={radius}
			>
...
export const IdentityVerificationModal = withErrorBoundary(

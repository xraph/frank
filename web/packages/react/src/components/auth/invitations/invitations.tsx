...
export interface InviteAcceptanceProps {
    token?: string;
    onAcceptSuccess?: (result: { organizationId: string; userId: string }) => void;
...
export interface InviteAcceptFormProps {
    invitation: InvitationData;
    onSubmit: (userData: { firstName?: string; lastName?: string; password?: string }) => void;
    isLoading?: boolean;
...
export interface InviteStatusProps {
    status: 'validating' | 'valid' | 'accepted' | 'declined' | 'expired' | 'invalid' | 'error';
    invitation?: InvitationData;
    error?: string;
    className?: string;
}
...
function useInvitation({
    ...
    const [status, setStatus] = useState<'idle' | 'validating' | 'valid' | 'accepting' | 'declining' | 'accepted' | 'declined' | 'expired' | 'invalid' | 'error'>('idle');
    ...
    return {
        ...
        acceptInvitation,
        declineInvitation,
        isLoading: status === 'validating' || status === 'accepting' || status === 'declining'
    };
}
...
function getInvitationStatus(expiresAt: string): 'active' | 'expiring' | 'expired' {
    const date = new Date(expiresAt);
    const now = new Date();
    const diffMs = date.getTime() - now.getTime();
...
export const InviteAcceptance = withErrorBoundary(function InviteAcceptance({
    ...
    const handleAccept = async (userData?: { firstName?: string; lastName?: string; password?: string }) => {
    ...
    const renderContent = () => {
        ...
        if (status === 'invalid' || status === 'error') {
            return <InviteStatus status={status === 'invalid' ? 'invalid' : 'error'} error={error || undefined} />;
        }

        if (status === 'expired') {
            return <InviteStatus status="expired" invitation={invitation || undefined} />;
        }

        if (status === 'accepted') {
            return <InviteStatus status="accepted" invitation={invitation || undefined} />;
        }

        if (status === 'declined') {
            return <InviteStatus status="declined" invitation={invitation || undefined} />;
        }
    ...
    return (
        <Card className={`max-w-md mx-auto ${className || ''}`} variant="shadow">
            <CardHeader className="flex flex-col items-center pb-2">
                ...
                <h2 className="text-xl font-bold text-center">Organization Invitation</h2>
                <Badge
                    color={invitationStatus === 'expired' ? 'danger' : invitationStatus === 'expiring' ? 'warning' : 'success'}
                    variant="flat"
                    className="mt-2"
                >
                    {expirationText}
            ...
            <CardBody className="space-y-6">
                ...
                <div className="text-center">
                    <div className="flex items-center justify-center mb-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-default-500">wants you to join their organization</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Badge color="primary" variant="flat" startContent={<ShieldCheckIcon className="h-3 w-3" />}>
                            {invitation.roleName}
                        </Badge>
                    </div>
                ...
                {invitation.message && (
                    <>
                        ...
                        <div className="bg-default-50 p-4 rounded-lg">
                            <p className="text-sm text-default-700 italic">"{invitation.message}"</p>
                        </div>
                    </>
                )}
        ...
        switch (status) {
            ...
            case 'error':
                return {
                    ...
                    title: 'Error',
                    message: error || 'An error occurred while processing your invitation.',
                    color: 'danger'
                };
            default:
                return {

...
import {Button as HeroButton, Checkbox, Divider, Input as HeroInput, Link, Select, SelectItem} from '@heroui/react';
...
type SignInStep = 'credentials' | 'mfa' | 'organization-select' | 'success' | 'email-verification';
...
const SuccessMessage = React.memo(({
            ...
            case 'magic-link-sent':
                return {
                    ...
                    extraButton: (
                        ...
                        }} size="sm">Resend magic link</Button>
                    ),
                };
    ...
    const showRedirectMessage = redirectUrl && !['email-verification-required', 'magic-link-sent'].includes(successType);
    ...
    return (
        <div className="text-center space-y-4">
            ...
            <div>
                ...
                {content.extraButton && <div className="flex justify-center py-4">
                    {content.extraButton}
                </div>}
            </div>
...
const OAuthButtons = React.memo(({
    ...
    const handleOAuthSignIn = useCallback(async (provider: string, signInMethod: () => Promise<void>) => {
        try {
            const result = await signInMethod();
            onSuccess(provider, { provider, ...result });
        } catch (error) {
            onError?.(error instanceof Error ? error : new Error(`${provider} sign-in failed`));
        }
    }, [onSuccess, onError]);

    const handleGoogleSignIn = useCallback(() => {
        handleOAuthSignIn('Google', () => signInWithGoogle({ redirectUrl, organizationId }));
    }, [handleOAuthSignIn, signInWithGoogle, redirectUrl, organizationId]);

    const handleGitHubSignIn = useCallback(() => {
        handleOAuthSignIn('GitHub', () => signInWithGitHub({ redirectUrl, organizationId }));
    }, [handleOAuthSignIn, signInWithGitHub, redirectUrl, organizationId]);

    const handleMicrosoftSignIn = useCallback(() => {
        handleOAuthSignIn('Microsoft', () => signInWithMicrosoft({ redirectUrl, organizationId }));
    }, [handleOAuthSignIn, signInWithMicrosoft, redirectUrl, organizationId]);

    const handleAppleSignIn = useCallback(() => {
        handleOAuthSignIn('Apple', () => signInWithApple({ redirectUrl, organizationId }));
...
const MagicLinkSection = React.memo(({
    ...
    const { sendMagicLink, isLoading, lastSentEmail, canResend } = useMagicLink();
    const [sent, setSent] = useState(false);

    const { components } = useConfig();
    const Button = components.Button ?? HeroButton;
    // Memoized envelope icon
    const envelopeIcon = useMemo(() => <EnvelopeIcon className="w-4 h-4" />, []);
    ...
    const handleSendMagicLink = useCallback(async () => {
        ...
        try {
            ...
            onError?.(error instanceof Error ? error : new Error('Failed to send magic link'));
        }
...
export function SignInForm({
    ...
    const { features, organizationSettings, components, linksPath, titleAlignment: configTitleAlignment } = useConfig();
    const { authenticateWithPasskey } = usePasskeys();

    // Memoized icons to prevent recreation
    const envelopeIcon = useMemo(() => <EnvelopeIcon className="w-4 h-4 text-default-400" />, []);
    const lockIcon = useMemo(() => <LockClosedIcon className="w-4 h-4 text-default-400" />, []);
    const lockIconSmall = useMemo(() => <LockClosedIcon className="w-4 h-4" />, []);
    const textAlign = React.useMemo(() => titleAlignment ?? configTitleAlignment, [titleAlignment, configTitleAlignment])
    ...
    if (CustomSignInForm) {
        return <CustomSignInForm {...{
            methods, email: initialEmail, organizationId: initialOrganizationId, redirectUrl,
            onSuccess, onError, showSignUpLink, showForgotPasswordLink, showOrganizationSelector,
            title, subtitle, variant, size, className, showBranding, disabled, footer, header
        }} />;
    }
    ...
    const stableFormWrapperProps = useMemo(() => {
        ...
        const getDefaultContent = () => {
            if (organizationSettings?.branding?.customLoginText) {
                return {
                    title: organizationSettings.branding.customLoginText.title || 'Welcome back',
                    subtitle: organizationSettings.branding.customLoginText.subtitle || 'Sign in to your account',
                };
            }
            return {
                title: 'Welcome back',
    ...
    }, [state.email, state.password, state.selectedOrganization, initialOrganizationId, signIn, onSuccess, onError, redirectUrl]);
    ...
    const handleMFAVerification = useCallback(async (code: string) => {
        ...
        try {
            ...
            const authError = error instanceof Error ? error : new Error('MFA verification failed');
            setFormError(authError.message);
            onError?.(authError);
        }
    ...
    const handleMagicLinkSuccess = useCallback((result: any) => {
        if (result.type === 'magic-link-sent') {
            setState(prev => ({
                ...
                step: 'success',
                successType: 'magic-link-sent',
                successData: { email: result.email, message: result.message, sendMagicLink: result.sendMagicLink, canResend: result.canResend }
            }));
        } else {
            setState(prev => ({
                ...prev,
    ...
    const handlePasskeyAuth = useCallback(async () => {
        try {
            ...
            const authError = error instanceof Error ? error : new Error('Passkey authentication failed');
            setFormError(authError.message);
            onError?.(authError);
        }
    ...
    return (
        <FormWrapper {...stableFormWrapperProps} onSubmit={handleSubmit} titleAlignment={textAlign}>
            {/* OAuth Buttons */}
                ...
                <div className="space-y-2">
                    ...
                    {showOrganizationSelector && organizationMemberships.length > 1 && (
                        <Select
                            ...
                            selectedKeys={state.selectedOrganization ? [state.selectedOrganization] : []}
            ...
            <div className="space-y-2">
                ...
                {availableMethods.includes('passkey') && (
                    <>
                        {(availableMethods.includes('password') && (availableMethods.includes('magic-link')) && !state.email) && (
                            <div className="relative">
                                <Divider className="my-4" />
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="bg-background px-2 text-sm text-default-500">or</span>

...
export function SignUpCard({
    ...
    if (CustomSignUpCard) {
        return <CustomSignUpCard {...{
            ...
            email, organizationId, invitationToken, redirectUrl, onSuccess, onError,
            title, subtitle, size, showBranding, disabled, requireTerms, termsUrl,
            privacyUrl, footer, header, maxWidth, centered
        }} />;
    }
    ...
    const cardContent = (
        ...
        >
            {/* Card Header */}
            <CardHeader className={`flex flex-col items-center text-center gap-3 p-${padding}`}>
                {header || (
                    <>
                        {/* Organization Logo */}
                        {orgBranding?.logoUrl && (
...
export interface InvitationSignUpCardProps extends Omit<SignUpCardProps, 'invitationToken' | 'organizationId'> {
    /**
     * Invitation token
     */
...
export function InvitationSignUpCard({
    ...
    const customHeader = header || (showInvitationDetails && invitationData && (
            ...
            <div className="space-y-2">
                {invitationData.inviterName && (
                    <p className="text-default-600">
                        ...
                        <span className="font-medium text-primary-600">{invitationData.role}</span>
                    </p>
                )}
                ...
                {isExpiringSoon && (
                    <div className="bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 rounded-lg p-3">
                        <p className="text-warning-700 dark:text-warning-400 text-sm">
                            ⚠️ This invitation expires soon. Please sign up as soon as possible.
                        </p>
                    </div>
                )}
            </div>
...
export function TrialSignUpCard({
    ...
    const customFooter = footer || (
        <div className="text-center text-xs text-default-500">
            {requiresCreditCard && (
                <p>
                    Your trial starts today. You can cancel anytime before it ends to avoid charges.
                </p>
            )}
        </div>
    );

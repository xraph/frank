...
function DefaultErrorFallback({
    ...
    const getErrorIcon = () => {
        switch (errorType) {
            ...
            case 'auth':
                return (
                    <svg className="w-12 h-12 text-danger-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {showDetails && showDetailsState && (
                            ...
                            >
                                <div className="space-y-2">
                                    <div>
                                        <span className="text-xs font-medium text-default-600">Error Message:</span>
                                    ...
                                    {error.stack && (
                                        <div>
                                            <span className="text-xs font-medium text-default-600">Stack Trace:</span>
...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
    ...
    componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
        const { onError, logErrors = true, reportErrors = false, errorReportingUrl } = this.props;
...
export function AuthErrorBoundary({ children, ...props }: Omit<ErrorBoundaryProps, 'title' | 'subtitle'>) {
    return (
        <ErrorBoundary
            title="Authentication Error"
...
export function FormErrorBoundary({ children, ...props }: Omit<ErrorBoundaryProps, 'title' | 'subtitle'>) {
    return (
        <ErrorBoundary
            title="Form Error"
...
export function ApiErrorBoundary({ children, ...props }: Omit<ErrorBoundaryProps, 'title' | 'subtitle'>) {
    return (
        <ErrorBoundary
            title="Service Error"

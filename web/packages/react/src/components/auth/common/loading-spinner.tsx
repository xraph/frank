...
const SpinnerVariant = ({ size, color, duration = 1 }: { size: string; color: string; duration?: number }) => (
    <motion.div
...
const DotsVariant = ({ size, color, duration = 1.4 }: { size: string; color: string; duration?: number }) => {
    const dotSizes = {
        'w-2 h-2': 'w-2 h-2',
        'w-3 h-3': 'w-3 h-3',
        'w-4 h-4': 'w-4 h-4',
...
const PulseVariant = ({ size, color, duration = 1.5 }: { size: string; color: string; duration?: number }) => (
    <motion.div
...
const BarsVariant = ({ size, color, duration = 1.2 }: { size: string; color: string; duration?: number }) => {
    const barHeights = {
        'w-2 h-2': 'w-1 h-4',
        'w-3 h-3': 'w-1 h-6',
        'w-4 h-4': 'w-1 h-8',
...
export function LoadingSpinner({
    ...
    if (CustomLoadingSpinner) {
        return <CustomLoadingSpinner {...{
            variant, size, color, text, showText, textPosition, centered, overlay,
            className, children, duration, backdrop, backdropOpacity
        }} />;
    }
    ...
    const renderSpinner = () => {
        ...
        switch (variant) {
            case 'dots':
                return <DotsVariant size={sizeClass} color={colorClass} duration={duration} />;
            case 'pulse':
                return <PulseVariant size={sizeClass} color={colorClass} duration={duration} />;
            case 'bars':
                return <BarsVariant size={sizeClass} color={colorClass} duration={duration} />;
            case 'custom':
                return children || <SpinnerVariant size={sizeClass} color={colorClass} duration={duration} />;
            case 'spinner':
            default:
    ...
    const renderText = () => {
        ...
        return (
            <span className={`${textSizes[size]} text-${color}-600 dark:text-${color}-400 font-medium`}>
        {text}
      </span>
        );
    };
    ...
    const renderContent = () => {
        ...
        return (
            <div className={`flex items-center justify-center ${flexDirection[textPosition]} ${gap[textPosition]}`}>
                {spinner}
                {textElement}
            </div>
        );
...
export function LoadingButton({
    ...
    if (CustomLoadingButton) {
        return <CustomLoadingButton {...{
            isLoading, loadingText, spinnerProps, variant, color, size, children, disabled, className, ...props
        }} />;
    }
...
export function LoadingProvider({ children }: { children: React.ReactNode }) {
    ...
    const setLoading = React.useCallback((loading: boolean, text = 'Loading...') => {

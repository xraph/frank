...
function OAuthButton({
    ...
    if (CustomOAuthButton) {
        return <CustomOAuthButton {...{
            provider, variant, size, showIcon, showName, text, onClick, disabled,
            isLoading, className, animated, buttonProps
        }} />;
    }
    ...
    const buttonContent = (
        <>
          ...
          {config.icon}
            ...
            {showName && (
                <span className={`font-medium ${isLoading ? 'opacity-70' : ''} ${!showIcon ? '' : 'ml-2'}`}>
...
export function OAuthButtons({
    ...
    if (CustomOAuthButtons) {
        return <CustomOAuthButtons {...{
            providers, layout, variant, size, showIcons, showNames, textFormat, customText,
            redirectUrl, organizationId, onSuccess, onError, className, disabled, animated, buttonProps
        }} />;
    }
    ...
    const handleProviderClick = React.useCallback(async (provider: OAuthProviderType) => {
        ...
        try {
            ...
            const authError = error instanceof Error ? error : new Error('OAuth authentication failed');
            onError?.(provider, authError);
        }
    }, [signInWithProvider, redirectUrl, organizationId, onSuccess, onError, disabled, isLoading]);

...
function TOTPSetup({ onSuccess, onError, onClose, isOpen }: TOTPSetupProps) {
    ...
    const handleSetup = async () => {
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to setup TOTP';
            onError(message);
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="space-y-4">
                ...
                <div className="text-center space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-default-500 mt-2">
                            We'll help you setup two-factor authentication using an authenticator app like Google Authenticator or Authy.
                        </p>
                    </div>

                    <div className="text-left space-y-3">
                        <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-6 h-6 bg-primary text-white text-xs rounded-full flex-shrink-0">1</div>
                            <p className="text-sm">Install an authenticator app on your phone</p>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-6 h-6 bg-default-300 text-default-600 text-xs rounded-full flex-shrink-0">2</div>
                            <p className="text-sm">Scan the QR code or enter the secret key</p>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="flex items-center justify-center w-6 h-6 bg-default-300 text-default-600 text-xs rounded-full flex-shrink-0">3</div>
            ...
            {step === 'verify' && setupData && (
                <div className="space-y-4">
                    ...
                    {setupData.secret && (
                        <div>
                            <p className="text-sm font-medium mb-2">Or enter this secret key manually:</p>
...
function SMSSetup({ onSuccess, onError, onClose, isOpen }: SMSSetupProps) {
    ...
    const handleSetup = async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to setup SMS';
            onError(message);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to verify SMS';
            onError(message);
        } finally {
            setIsLoading(false);
        }
...
export function MFASetup({
    ...
    if (CustomMFASetup) {
        return <CustomMFASetup {...{
            onSuccess, onError, showTOTPSetup, showSMSSetup, showEmailSetup,
            showBackupCodes, className, isDisabled, variant, size, defaultMethod,
            hideMethods, customMethods
        }} />;
    }
    ...
    const handleRemoveMethod = async (methodId: string) => {
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to remove MFA method';
            onError?.(message);
        }
    };
                    ...
                    <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-3">
                            <div className={`flex items-center justify-center w-10 h-10 rounded-lg ${
                                ...
                                <svg className={`w-5 h-5 ${isEnabled ? 'text-success' : 'text-default-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {isEnabled && (
                                        <span className="text-xs text-default-500">
                                            {mfaMethods.length} method{mfaMethods.length !== 1 ? 's' : ''} configured
                                        </span>
                                    )}
                                </div>
                            </div>
                    ...
                    <>
                        ...
                        <CardBody>
                            <p className="text-sm text-default-600">
                                ...
                                Choose from authenticator apps, SMS, or backup codes.
                            </p>
                        </CardBody>
                    </>
                )}
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Button
                                                ...
                                                onPress={hasTOTP ?
                                                    () => handleRemoveMethod(mfaMethods.find(m => m.type === 'totp')?.id || '') :
                                                    totpModal.onOpen
                                                }
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Button
                                                ...
                                                onPress={hasSMS ?
                                                    () => handleRemoveMethod(mfaMethods.find(m => m.type === 'sms')?.id || '') :
                                                    smsModal.onOpen
                                                }
                                                isDisabled={isDisabled}
                                            >

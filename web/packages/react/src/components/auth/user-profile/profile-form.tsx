...
export function ProfileForm({
    ...
    if (CustomProfileForm) {
        return <CustomProfileForm {...{
            onSubmit, onUpdate, onSuccess, onError, showSaveButton, autoSave,
            autoSaveDelay, showImageUpload, showEmail, showPhone, showUsername,
            showMetadata, customFields, hideFields, fieldLayout, variant, size,
            isDisabled, isLoading: externalLoading, className, showVerificationStatus,
            allowImageRemoval, onImageUpload, onImageRemove, maxImageSize, allowedImageTypes
        }} />;
    }
    ...
    const handleCustomFieldChange = React.useCallback((field: string, value: any) => {
        setFormData(prev => ({
            ...prev,
            customFields: {
                ...prev.customFields,
    ...
    const validateForm = React.useCallback(() => {
        ...
        customFields.forEach(field => {
            ...
            if (field.validation && formData.customFields?.[field.key]) {
                const validationError = field.validation(formData.customFields[field.key]);
                if (validationError) {
    ...
    const handleImageUpload = React.useCallback(async (file: File) => {
        ...
        if (file.size > maxImageSize) {
            onError?.(`Image size must be less than ${Math.round(maxImageSize / 1024 / 1024)}MB`);
            return;
        }
    ...
    }, [allowedImageTypes, maxImageSize, onImageUpload, updateProfileImage, handleFieldChange, onSuccess, onError]);
    ...
    }, [onImageRemove, removeProfileImage, handleFieldChange, onSuccess, onError]);
    ...
    return (
        <form onSubmit={handleSubmit} className={`space-y-6 ${className}`}>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <p className="text-xs text-default-500">
                                    JPG, PNG or WebP. Max size {Math.round(maxImageSize / 1024 / 1024)}MB.
                                </p>
                            </div>
                        </div>
            ...
            <Card variant={variant}>
                ...
                <Divider />
                <CardBody>
                    <div className={`grid gap-4 ${fieldLayout === 'double' ? 'md:grid-cols-2' : 'grid-cols-1'}`}>
                        {/* First Name */}
                                    ...
                                    case 'switch':
                                        return (
                                            <div key={field.key} className="flex items-center justify-between">
                                                ...
                                                <Switch
                                                    isSelected={!!value}
                                                    onValueChange={(checked) => handleCustomFieldChange(field.key, checked)}
                                                    size={size}
                                                    isDisabled={isDisabled}
                                                />
                                            </div>

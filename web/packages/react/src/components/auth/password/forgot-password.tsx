...
export function ResetPassword({
    ...
    const verifyResetToken = useCallback(async () => {
        ...
        try {
            ...
            setTokenStatus('invalid');
            const errorMessage = err instanceof Error ? err.message : 'Failed to verify reset token';
            setError(errorMessage);
            onError?.(err instanceof Error ? err : new Error(errorMessage));
        }
    ...
    }, [password, confirmPassword, isPasswordValid, resetToken, signIn, onSuccess, onError, redirectUrl]);
        ...
        <FormWrapper {...formWrapperProps} onSubmit={handleSubmit}>
            <div className="space-y-4">
                ...
                <Button
                    ...
                    className="w-full"
                    isLoading={isLoading}
                    isDisabled={!password || !confirmPassword || !isPasswordValid || isLoading}
                >
 ...
 */
export function ForgotPasswordCard(props: Omit<ForgotPasswordProps, 'variant'>) {

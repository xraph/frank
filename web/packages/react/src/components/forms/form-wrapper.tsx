...
const FormStateProvider = React.memo(({ children, isSubmitting, initialErrors }: FormStateProviderProps) => {
    ...
    const setFieldError = useCallback((field: string, fieldError: string | string[] | null) => {
        if (fieldError === null) {
    ...
    const setFieldTouched = useCallback((field: string, fieldTouched = true) => {
...
const Messages = React.memo(({ success, error, disableAnimations }: MessagesProps) => {
...
const Header = React.memo(({ header, title, subtitle, showCard, desc, titleAlignment }: HeaderProps) => {
    ...
    if (!showCard) {
        return (
            <div className={cn("flex flex-col text-center pb-2", getTitleAlignment(titleAlignment ?? 'center'))}>
                {title && (
...
export function FormWrapper({
    ...
    if (CustomFormWrapper) {
        return <CustomFormWrapper {...{
            children, title, subtitle, header, footer, isSubmitting, error, success,
            onSubmit, initialErrors, className, variant, size, showCard, cardProps,
            loadingContent, logo, width, centered, disableAnimations
        }} />;
    }
    ...
    const logoElement = useMemo(() => (
        <Logo
            logo={logo}
            orgLogoUrl={organizationSettings?.branding?.logoUrl}
        />
    ...
    const formContent = useMemo(() => (
        <FormStateProvider isSubmitting={isSubmitting} initialErrors={initialErrors}>
            {logoElement}
            {messagesElement}
            <FormContent
                onSubmit={onSubmit}

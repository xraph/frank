...
import {Button, Chip, Input, Listbox, ListboxItem, Popover, PopoverContent, PopoverTrigger} from '@heroui/react';
...
function validateEmail(
    ...
    if (emailParts.length === 2) {
        ...
        if (domain && commonMisspellings[domain]) {
            suggestions.push(`Did you mean ${username}@${commonMisspellings[domain]}?`);
        }
        ...
        if (validateDomain && domain) {
            ...
            if (allowedDomains.length > 0 && !allowedDomains.includes(domain)) {
                errors.push(`Email must be from one of these domains: ${allowedDomains.join(', ')}`);
            }
...
export function EmailField({
    ...
    const RootInput = React.useMemo(() => components.Input ?? Input, [components.Input]);
    ...
    if (CustomEmailField) {
        return <CustomEmailField {...{
            ...
            showSuggestions, validateFormat, validateDomain, allowedDomains, blockedDomains,
            size, variant, className, autoFocus, autoComplete, error: externalError, description,
            showVerificationStatus, isVerified, onRequestVerification, startContent, endContent
        }} />;
    }
    ...
    const [showSuggestionPopover, setShowSuggestionPopover] = React.useState(false);
    ...
    const effectiveBlockedDomains = React.useMemo(() => {
        const orgBlockedDomains = organizationSettings?.emailRestrictions?.blockedDomains;
    ...
    }, [currentValue, validateFormat, validateDomain, effectiveAllowedDomains, effectiveBlockedDomains]);
        ...
        <div className={`space-y-2 ${className}`}>
            ...
            {effectiveAllowedDomains.length > 0 && (
                <div className="text-xs text-default-500">
                    <span className="font-medium">Allowed domains:</span> {effectiveAllowedDomains.join(', ')}
                </div>
            )}

            {/* Suggestions (non-popover) */}
            <AnimatePresence>
                {!showSuggestionPopover && validation?.suggestions.length && isFocused && (
                    <motion.div

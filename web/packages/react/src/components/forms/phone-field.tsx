...
function parsePhoneNumber(phoneNumber: string, country: Country): {
    isValid: boolean;
    e164: string;
    national: string;
    formatted: string;
...
export function PhoneField({
    ...
    if (CustomPhoneField) {
        return <CustomPhoneField {...{
            name, label, placeholder, value, onChange, onBlur, onFocus, defaultCountry,
            preferredCountries, allowedCountries, blockedCountries, required, disabled,
            validateFormat, size, variant, className, autoFocus, autoComplete,
            error: externalError, description, showVerificationStatus, isVerified,
            onRequestVerification, endContent
        }} />;
    }
    ...
    const effectiveAllowedCountries = React.useMemo(() => {
        const orgCountries = organizationSettings?.phoneRestrictions?.allowedCountries;
    ...
    const effectiveBlockedCountries = React.useMemo(() => {
        const orgBlockedCountries = organizationSettings?.phoneRestrictions?.blockedCountries;
    ...
    const availableCountries = React.useMemo(() => {
        ...
        if (effectiveAllowedCountries.length > 0) {
            filtered = filtered.filter(c => effectiveAllowedCountries.includes(c.code));
        }

        if (effectiveBlockedCountries.length > 0) {
            filtered = filtered.filter(c => !effectiveBlockedCountries.includes(c.code));
        }

        // Sort with preferred countries first
        const preferred = filtered.filter(c => preferredCountries.includes(c.code));
        const others = filtered.filter(c => !preferredCountries.includes(c.code));

        return [...preferred, ...others];
    }, [effectiveAllowedCountries, effectiveBlockedCountries, preferredCountries]);
    ...
    const handleChange = React.useCallback((newValue: string) => {
        ...
        if (onChange && validation) {
            ...
            const e164 = cleaned.startsWith('+') ? cleaned : selectedCountry.dialCode + cleaned.replace(/^\d/, '');
    ...
    const verificationContent = React.useMemo(() => {
        ...
        return null;
    }, [showVerificationStatus, endContent, isVerified, currentValue, validation, onRequestVerification]);
    ...
    const countrySelector = (
        <Select
            ...
            placeholder="Country"
            renderValue={(items) => {
                const country = availableCountries.find(c => c.code === selectedCountry.code);
                return country ? (
    ...
    return (
        <div className={`space-y-2 ${className}`}>
            ...
            {effectiveAllowedCountries.length > 0 && (
                <div className="text-xs text-default-500">
                    <span className="font-medium">Allowed countries:</span> {effectiveAllowedCountries.join(', ')}
                </div>
            )}
        </div>
    );

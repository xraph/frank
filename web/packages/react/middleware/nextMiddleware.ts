...
export interface NextAuthMiddlewareOptions {
	...
	beforeAuth?: (request: NextRequest, response: Response) => Promise<NextResponse | null>; // Before auth hook
	afterAuth?: (request: NextRequest, response: Response, isAuthenticated: boolean, user?: User) => Promise<NextResponse | null>; // After auth hook
	onAuthSuccess?: (request: NextRequest, response: Response, user: User) => Promise<NextResponse | null>; // Called when auth succeeds
	onAuthFailure?: (request: NextRequest, response: Response) => Promise<NextResponse | null>; // Called when auth fails
}
...
const validateSession = async (cookie: string): Promise<{ isValid: boolean, user?: any }> => {
	try {
...
export function createNextAuthMiddleware(customOptions: NextAuthMiddlewareOptions = {}) {
	...
	return async function middleware(request: NextRequest) {
		...
		if (
			matchPath(pathname, options.publicPages || [])
		) {
			return response;
		}
		...
		if (!isAuthenticated) {
			...
			if (isTokenExpired(cookieHeader)) {
				const newTokenData = await refreshAuthToken(cookieHeader, cookieHandler)
				if (!newTokenData) {
		...
		if (options.afterAuth) {
			const rsp = await options.afterAuth(request, response, isAuthenticated, user);
			if (rsp) return rsp;
		}
		...
		if (isAuthenticated && matchPath(pathname, options.authPages || [])) {
			...
			await cookieHandler.copyTo(new NextServerCookieHandler(request, redirectResponse));
		...
		if (options.orgRequired && isAuthenticated) {
			...
			if (!orgId && !pathname.includes('/organization-select')) {
				const redirectResponse = NextResponse.redirect(new URL('/organization-select', request.url), {
					headers: requestHeaders
				});
				await cookieHandler.copyTo(new NextServerCookieHandler(request, redirectResponse));
				return redirectResponse;

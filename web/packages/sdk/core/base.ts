import {
    AuthenticationApi,
    Configuration, type InitOverrideFunction, type LoginResponse, type RefreshTokenResponse,
...
import {
    ...
    FrankAuthError, HybridAuthStorage,
    StorageAdapter,
    StorageManager
} from './index';
...
export class BaseSDK {
    ...
    constructor(config: FrankAuthConfig, accessToken?: string) {
        this._hybridAuthStorage = new HybridAuthStorage(
     ...
     */
    public checkTokenExpiration(token: string | null, bufferSeconds: number = this.TOKEN_REFRESH_BUFFER): TokenExpirationResult {
        if (!token) {
            return {
                isExpired: true,
                isValid: false,
     ...
     * @returns Token expiration result
     */
    public isAccessTokenExpired(bufferSeconds: number = this.TOKEN_REFRESH_BUFFER): TokenExpirationResult {
     ...
     * @returns Token expiration result
     */
    public isRefreshTokenExpired(bufferSeconds: number = this.TOKEN_REFRESH_BUFFER): TokenExpirationResult {
     ...
     */
    public async verifyAndRenewRefreshToken(forceRenew: boolean = false): Promise<{
        renewed: boolean;
        wasExpired: boolean;
        newTokens?: {
            accessToken: string;
        ...
        if (!refreshTokenResult.isValid) {
            throw new FrankAuthError('Invalid refresh token', 'INVALID_REFRESH_TOKEN');
        }
        ...
        if (!this._refreshToken) {
            throw new FrankAuthError('No refresh token available', 'NO_REFRESH_TOKEN');
        }
    ...
    public analyzeTokenError(error: any): {
        ...
        } else if (accessTokenResult.isExpired && refreshTokenResult.isValid && !refreshTokenResult.isExpired) {
            recommendedAction = 'refresh';
    ...
    private async refreshTokenAndRetry<T>(
        ...
        while (retryCount <= maxRetries) {
            try {
                ...
                retryCount++;
                console.log(`Retrying API call after token refresh (attempt ${retryCount}/${maxRetries})`);
            }
        }
    ...
    private async renewRefreshToken(token?: string, initOverrides?: RequestInit | InitOverrideFunction): Promise<RefreshTokenResponse> {
    ...
    public getDiagnostics(): {
        ...
        const canRefreshTokens = this._refreshToken !== null &&

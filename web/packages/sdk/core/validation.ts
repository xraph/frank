 ...
 */
export function createError(field: string, message: string, value?: any): ConfigValidationError {
    return {
        field,
        message,
        value,
 ...
 */
export function createWarning(field: string, message: string, value?: any): ConfigValidationError {
    return {
        field,
        message,
        value,
...
export function validateSecretKey(key?: string): ConfigValidationError[] {
    ...
    if (!/^sk_(test|live)_[a-zA-Z0-9_]+$/.test(key)) {
        errors.push(createError('secretKey', 'Invalid secret key format. Expected: sk_test_... or sk_live_...', key));
    }
    if (key.length < 20) {
        errors.push(createError('secretKey', 'Secret key appears to be too short', key));
    }
    return errors;
}
...
export function validateProjectId(id?: string): ConfigValidationError[] {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(id)) {
        errors.push(createError('projectId', 'Project ID contains invalid characters', id));
    }
    if (id.length < 3) {
        errors.push(createError('projectId', 'Project ID appears to be too short', id));
    }
    return errors;
}
 ...
 */
export function validateStorageKeyPrefix(prefix?: string): ConfigValidationError[] {
    const errors: ConfigValidationError[] = [];
    if (!prefix) {
        return errors; // Storage key prefix is optional
    }
    if (typeof prefix !== 'string') {
        errors.push(createError('storageKeyPrefix', 'Storage key prefix must be a string', prefix));
        return errors;
    }
    if (prefix.includes(' ')) {
        errors.push(createWarning('storageKeyPrefix', 'Storage key prefix contains spaces which may cause issues', prefix));
    }
    return errors;
}
 ...
 */
export function validateSessionCookieName(name?: string): ConfigValidationError[] {
    const errors: ConfigValidationError[] = [];
    if (!name) {
        return errors; // Session cookie name is optional
    }
    if (typeof name !== 'string') {
        errors.push(createError('sessionCookieName', 'Session cookie name must be a string', name));
        return errors;
    }
    // Cookie name validation
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
        errors.push(createError('sessionCookieName', 'Session cookie name contains invalid characters', name));
    }
    return errors;
}
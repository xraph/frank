...
func (mq *MembershipQuery) QueryRole() *RoleQuery {
...
func (mq *MembershipQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (mq *MembershipQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (mq *MembershipQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (mq *MembershipQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (mq *MembershipQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (mq *MembershipQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MembershipQuery) WithRole(opts ...func(*RoleQuery)) *MembershipQuery {
	query := (&RoleClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MembershipQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Membership, init func(*Membership), assign func(*Membership, *Role)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*Membership)
	for i := range nodes {
		fk := nodes[i].RoleID
...
func (mq *MembershipQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(membership.Table, membership.Columns, sqlgraph.NewFieldSpec(membership.FieldID, field.TypeString))
	_spec.From = mq.sql

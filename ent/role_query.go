...
type RoleQuery struct {
	...
	withUserAssignments      *UserRoleQuery
	withSystemUsers          *UserQuery
	withPermissions          *PermissionQuery
	withMemberships          *MembershipQuery
	withParent               *RoleQuery
	withChildren             *RoleQuery
	modifiers                []func(*sql.Selector)
	withNamedUserAssignments map[string]*UserRoleQuery
	withNamedSystemUsers     map[string]*UserQuery
	withNamedPermissions     map[string]*PermissionQuery
	withNamedMemberships     map[string]*MembershipQuery
	withNamedChildren        map[string]*RoleQuery
...
func (rq *RoleQuery) QueryUserAssignments() *UserRoleQuery {
...
// QuerySystemUsers chains the current query on the "system_users" edge.
func (rq *RoleQuery) QuerySystemUsers() *UserQuery {
...
func (rq *RoleQuery) QueryMemberships() *MembershipQuery {
	query := (&MembershipClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withUserAssignments: rq.withUserAssignments.Clone(),
		withSystemUsers:     rq.withSystemUsers.Clone(),
		withPermissions:     rq.withPermissions.Clone(),
		withMemberships:     rq.withMemberships.Clone(),
...
func (rq *RoleQuery) WithUserAssignments(opts ...func(*UserRoleQuery)) *RoleQuery {
	query := (&UserRoleClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withUserAssignments = query
	return rq
}

// WithSystemUsers tells the query-builder to eager-load the nodes that are connected to
// the "system_users" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithSystemUsers(opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withSystemUsers = query
	return rq
}
...
func (rq *RoleQuery) WithMemberships(opts ...func(*MembershipQuery)) *RoleQuery {
	query := (&MembershipClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withMemberships = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	var (
		...
		loadedTypes = [7]bool{
			...
			rq.withUserAssignments != nil,
			rq.withSystemUsers != nil,
			rq.withPermissions != nil,
			rq.withMemberships != nil,
			rq.withParent != nil,
			rq.withChildren != nil,
		}
	)
	...
	if query := rq.withUserAssignments; query != nil {
		if err := rq.loadUserAssignments(ctx, query, nodes,
			func(n *Role) { n.Edges.UserAssignments = []*UserRole{} },
			func(n *Role, e *UserRole) {
	...
	if query := rq.withSystemUsers; query != nil {
		if err := rq.loadSystemUsers(ctx, query, nodes,
			func(n *Role) { n.Edges.SystemUsers = []*User{} },
			func(n *Role, e *User) { n.Edges.SystemUsers = append(n.Edges.SystemUsers, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := rq.withMemberships; query != nil {
		if err := rq.loadMemberships(ctx, query, nodes,
			func(n *Role) { n.Edges.Memberships = []*Membership{} },
			func(n *Role, e *Membership) {
	...
	for name, query := range rq.withNamedUserAssignments {
		if err := rq.loadUserAssignments(ctx, query, nodes,
			func(n *Role) { n.appendNamedUserAssignments(name) },
			func(n *Role, e *UserRole) {
	...
	for name, query := range rq.withNamedSystemUsers {
		if err := rq.loadSystemUsers(ctx, query, nodes,
			func(n *Role) { n.appendNamedSystemUsers(name) },
			func(n *Role, e *User) { n.appendNamedSystemUsers(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range rq.withNamedMemberships {
		if err := rq.loadMemberships(ctx, query, nodes,
			func(n *Role) { n.appendNamedMemberships(name) },
			func(n *Role, e *Membership) {
	...
	for name, query := range rq.withNamedChildren {
		if err := rq.loadChildren(ctx, query, nodes,
			func(n *Role) { n.appendNamedChildren(name) },
			func(n *Role, e *Role) {
...
func (rq *RoleQuery) loadUserAssignments(ctx context.Context, query *UserRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *UserRole)) error {
	...
	query.Where(predicate.UserRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.UserAssignmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (rq *RoleQuery) loadSystemUsers(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.SystemUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(role.SystemUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.SystemUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.SystemUsersPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "system_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (rq *RoleQuery) loadMemberships(ctx context.Context, query *MembershipQuery, nodes []*Role, init func(*Role), assign func(*Role, *Membership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[xid.ID]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(membership.FieldRoleID)
	}
	query.Where(predicate.Membership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.MembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (rq *RoleQuery) WithNamedUserAssignments(name string, opts ...func(*UserRoleQuery)) *RoleQuery {
	query := (&UserRoleClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedUserAssignments == nil {
		rq.withNamedUserAssignments = make(map[string]*UserRoleQuery)
	}
	rq.withNamedUserAssignments[name] = query
	return rq
}

// WithNamedSystemUsers tells the query-builder to eager-load the nodes that are connected to the "system_users"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedSystemUsers(name string, opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedSystemUsers == nil {
		rq.withNamedSystemUsers = make(map[string]*UserQuery)
	}
	rq.withNamedSystemUsers[name] = query
	return rq
}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedMemberships(name string, opts ...func(*MembershipQuery)) *RoleQuery {
	query := (&MembershipClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedMemberships == nil {
		rq.withNamedMemberships = make(map[string]*MembershipQuery)
	}
	rq.withNamedMemberships[name] = query
	return rq
}

// WithNamedChildren tells the query-builder to eager-load the nodes that are connected to the "children"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithNamedChildren(name string, opts ...func(*RoleQuery)) *RoleQuery {
	query := (&RoleClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedChildren == nil {
		rq.withNamedChildren = make(map[string]*RoleQuery)
	}

...
func (akq *ApiKeyQuery) QueryOrganization() *OrganizationQuery {
	query := (&OrganizationClient{config: akq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := akq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (akq *ApiKeyQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (akq *ApiKeyQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (akq *ApiKeyQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (akq *ApiKeyQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (akq *ApiKeyQuery) IDs(ctx context.Context) (ids []xid.ID, err error) {
...
func (akq *ApiKeyQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := akq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (akq *ApiKeyQuery) WithOrganization(opts ...func(*OrganizationQuery)) *ApiKeyQuery {
	query := (&OrganizationClient{config: akq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (akq *ApiKeyQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*ApiKey, init func(*ApiKey), assign func(*ApiKey, *Organization)) error {
	ids := make([]xid.ID, 0, len(nodes))
	nodeids := make(map[xid.ID][]*ApiKey)
	for i := range nodes {
...
func (akq *ApiKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := akq.ctx.Fields; len(fields) > 0 {
		...
		if akq.withOrganization != nil {
			_spec.Node.AddColumnOnce(apikey.FieldOrganizationID)
		}
	}
